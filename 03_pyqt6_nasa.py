"""
NASA GUI Example - PyQt6
========================
PyQt6 is a comprehensive set of Python bindings for Qt6, providing access to 
a powerful cross-platform GUI toolkit with extensive widget support.
...
class NASAControlPanel(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("NASA Control Panel - PyQt6")
    ...
    def _get_stylesheet(self):
        return f"""
            ...
            QRadioButton::indicator:checked {{
                border: 2px solid {self.nasa_red};
                border-radius: 10px;
                background-color: {self.nasa_red};
            }}
            QPushButton {{
                padding: 10px 20px;
                border-radius: 5px;
    ...
    def setup_ui(self):
        ...
        header_frame.setStyleSheet(f"""
            QFrame {{
                background-color: {self.nasa_blue};
                border-radius: 10px;
            }}
        """)
        ...
        for i, (label_text, key) in enumerate(readouts):
            ...
            label.setStyleSheet("color: #888899; font-size: 11px; background: transparent;")
            container_layout.addWidget(label)
            
            display = DigitalDisplay("000000" if key != "fuel" and key != "temperature" else "100.0" if key == "fuel" else "072.5")
        ...
        self.status_label.setStyleSheet(f"color: {self.digital_green}; background: transparent;")
    ...
    def abort_mission(self):
        self.current_mode = "standby"
        # Find and check the standby radio button
